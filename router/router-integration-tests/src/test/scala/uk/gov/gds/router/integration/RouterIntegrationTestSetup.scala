...
trait RouterIntegrationTestSetup
  ...
  protected def createRoute(applicationId: String, incomingPath: String, routeType: String, host: String = "govuk") =
    post("/routes/" + host + "/" + incomingPath, Map("application_id" -> applicationId, "route_type" -> routeType))
