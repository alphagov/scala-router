...
class HttpClientTest < MiniTest::Unit::TestCase
  ...
  def test_returns_correct_responses_for_full_route_manipulations
    ...
    response = @http_client.put("/routes/test_host/test_route/test", {route_type: "full", application_id: "test_application"})
    assert_equal("201", response.code)
  ...
  def test_returns_correct_response_for_prefix_route_manipulations
    ...
    response = @http_client.put("/routes/test_host/test_route", route_type: "prefix", application_id: "test_application")
    assert_equal("201", response.code)
  ...
  def test_returns_error_response_when_creating_prefix_route_with_more_than_one_segment
    ...
    response = @http_client.put("/routes/test_host/test_route/test", { route_type: "prefix", application_id: "test_application" })
    assert_equal("500", response.code)
  end

  def test_returns_error_response_when_creating_prefix_route_without_application  
    response = @http_client.put("/routes/test_host/test_route", { route_type: "prefix", application_id: "test_application_test1" })
    assert_equal("500", response.code)
  end

  def test_returns_error_response_when_creating_full_route_without_application
    response = @http_client.put("/routes/test_host/test_route/test", { route_type: "full", application_id: "test_application_test2" })
    assert_equal("500", response.code)
  end
